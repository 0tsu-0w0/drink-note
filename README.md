...
独自ドメインを持っていない場合の手順を
[docs/smtp-brevo.md](./docs/smtp-brevo.md) にまとめました。
自分のメールアドレスを送信元として認証するだけで使え、DNS の設定は要りません。

ドメインを持っている場合は Resend も選べます（送信元が自分のドメインになり、
到達率が上がります。SPF・DKIM の DNS 設定が必要です）。

設定後は **Authentication → Rate Limits** で送信の上限を引き上げてください。
標準送信のときの数通のままだと、SMTP を繋いでも制限に当たります。
